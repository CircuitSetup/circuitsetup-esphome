...
static void reset_gdo_status(void);
...
esp_err_t gdo_init(const gdo_config_t *config) {
    ...
    g_config = *config;
    g_use_legacy_break_fallback = false;
    reset_gdo_status();
    ...
    if (g_config.obst_in_pin >= 0 && !g_config.obst_from_status) {
        ...
        g_obst_stats = (gdo_obstruction_stats_t*)calloc(1, sizeof(gdo_obstruction_stats_t));
        if (!g_obst_stats) {
            err = ESP_ERR_NO_MEM;
            goto fail;
        }
        ...
        err = gpio_isr_handler_add(g_config.obst_in_pin, obst_isr_handler, (void*)g_obst_stats);
        if (err != ESP_OK) {
            goto fail;
        }
        obst_isr_added = true;
        ...
        timer_args.arg = (void*)g_obst_stats;
        timer_args.dispatch_method = ESP_TIMER_TASK;
...
fail:
    ...
    if (obst_isr_added) {
        gpio_isr_handler_remove(g_config.obst_in_pin);
    }
...
esp_err_t gdo_deinit(void) {
    ...
    tmp_err = uart_driver_delete(g_config.uart_num);
    if (tmp_err != ESP_OK && tmp_err != ESP_ERR_INVALID_STATE) {
        err = tmp_err;
    }
...
static void gdo_sync_task(void* arg) {
    bool synced = true;
    esp_timer_handle_t v1_status_timer = NULL;
    ...
    if (g_status.protocol != GDO_PROTOCOL_SEC_PLUS_V2) {
        ...
        if (g_status.door == GDO_DOOR_STATE_UNKNOWN) {
            ...
            if (esp_timer_create(&timer_args, &v1_status_timer) != ESP_OK) {
                ...
                uart_flush(g_config.uart_num);
                xQueueReset(gdo_event_queue);
                if (esp_timer_start_periodic(v1_status_timer, 250 * 1000) != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to start V1 status timer");
                    synced = false;
                    goto done;
                }
            }
...
static esp_err_t schedule_event(gdo_event_type_t event, uint32_t time_us) {
    ...
    if (err != ESP_OK) {
        esp_timer_delete(event_arg->timer);
        free(event_arg);
    }
...
static esp_err_t transmit_packet(uint8_t *packet) {
    ...
    if (g_status.protocol == GDO_PROTOCOL_SEC_PLUS_V2) {
        /*
         * Security+ v2 requires a break before each packet.
         * Prefer the official UART API for break generation. If the driver/platform does not
         * support a zero-length break write, fallback to the legacy deterministic baud-rate method.
         */
        if (!g_use_legacy_break_fallback) {
            if (uart_write_bytes_with_break(g_config.uart_num, "", 0, 13) >= 0) {
                ...
                return uart_flush_input(g_config.uart_num);
            }

            g_use_legacy_break_fallback = true;
            ESP_LOGW(TAG, "uart_write_bytes_with_break unsupported, using legacy break workaround");
        }

        // Legacy fallback: generate ~13 bit-time break via temporary baud-rate shift.
...
static void decode_packet(uint8_t *packet) {
    ...
    static uint32_t last_decode_error_time = 0;
    static bool obst_clear_from_status = false;

    if (decode_wireline(packet, &rolling, &fixed, &data) != 0) {
        if (time_now - last_decode_error_time > 5000) {
            ESP_LOGW(TAG, "Ignoring invalid wireline packet");
            last_decode_error_time = time_now;
        }
        return;
    }
    ...
    if (remote_id == g_status.client_id) { // my commands
        ESP_LOGV(TAG, "Ignoring self packet: rolling=%07" PRIx32 " fixed=%010" PRIx64 " data=%08" PRIx32,
                 rolling, fixed, data);
        return;
    } else {
        ESP_LOGV(TAG, "RX packet: rolling=%07" PRIx32 " fixed=%010" PRIx64 " data=%08" PRIx32,
                 rolling, fixed, data);
    }
    ...
    ESP_LOGD(TAG, "cmd=%03x (%s) byte2=%02x byte1=%02x nibble=%01x",
    ...
    for (;;) {
        if (xQueueReceive(gdo_event_queue, (void*)&event, (TickType_t)portMAX_DELAY)) {
            ...
            case GDO_EVENT_TX_PENDING: {
                ...
                if (uxQueueMessagesWaiting(gdo_tx_queue) > 0 &&
                    queue_event((gdo_event_t){GDO_EVENT_TX_PENDING}) != ESP_OK) {
                    ESP_LOGW(TAG, "TX queue still has pending messages but follow-up TX event could not be queued");
                }
...
inline static void update_battery_state(gdo_battery_state_t battery_state) {
    gdo_battery_state_t normalized = GDO_BATT_STATE_UNKNOWN;
    if (battery_state == GDO_BATT_STATE_CHARGING || battery_state == GDO_BATT_STATE_FULL) {
        normalized = battery_state;
    }

    ESP_LOGD(TAG, "Battery state: %s", gdo_battery_state_to_string(normalized));
    if (normalized != g_status.battery) {
        g_status.battery = normalized;
...
static void drain_tx_queue(void) {
    ...
    gdo_tx_message_t tx_message = {0};
    while (xQueueReceive(gdo_tx_queue, &tx_message, 0) == pdTRUE) {
        free(tx_message.packet);
    }
}
...
inline static esp_err_t queue_event(gdo_event_t event) {
    static uint32_t last_queue_error_time = 0;
    uint32_t now = esp_timer_get_time() / 1000;

    if (!gdo_event_queue) {
        if (now - last_queue_error_time > 5000) {
            ESP_LOGE(TAG, "Event queue not created");
            last_queue_error_time = now;
        }
        return ESP_ERR_INVALID_STATE;
    }

    if (xQueueSend(gdo_event_queue, &event, pdMS_TO_TICKS(5)) == pdFALSE) {
        if (now - last_queue_error_time > 5000) {
            ESP_LOGW(TAG, "Event queue full; dropping event %d", event.gdo_event);
            last_queue_error_time = now;
        }
        return ESP_ERR_NO_MEM;
    }
